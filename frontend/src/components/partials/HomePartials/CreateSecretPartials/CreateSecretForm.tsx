...
const CreateSecretForm = forwardRef<HTMLDivElement>((_, ref) => {
  ...
  const secretKeyError =
    debouncedSecretKey.length > 0
      ? invalidSecretKey(debouncedSecretKey)
      : undefined;

  const formHasErrors = emailError || secretKeyError || secretError;
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const secretKeyErr = invalidSecretKey(secretFormData.secretKey);
    const secretErr = invalidSecretText(secretFormData.secret, charLimit);

    if (emailErr || secretErr || secretKeyErr) {
      return;
    }
