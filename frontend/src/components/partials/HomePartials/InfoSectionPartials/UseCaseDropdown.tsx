  ...
  return (
    <div>
      ...
      <div className="relative">
        ...
        <ul
          className={`absolute z-30 w-full bg-(--white) rounded-md px-2 h-max text-black transition-all duration-300 ${showDropdown ? "max-h-65 top-13 opacity-100 py-2 " : "max-h-0 py-0 top-11 opacity-0 pointer-events-none"}`}
        >
          {Object.entries(useCases).map(([key, value], index) => (
            <li
              key={key}
              className={`${key === selectedCase ? "bg-(--gray)/80 outline outline-(--white)" : "bg-(--white) cursor-pointer hover:bg-(--gray)/50"} py-2 px-2 rounded-md transition-opacity duration-300 ${showDropdown ? "opacity-100" : "opacity-0"}`}
              style={{
                transitionDelay: showDropdown ? `${100 + index * 40}ms` : "0ms",
              }}
              onClick={() => {
