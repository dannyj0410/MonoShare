    ...
    >
      ...
      <div className="relative w-full h-full flex flex-col items-center">
        ...
        <m.div
          ...
          className="absolute inset-0 bg-[url('/clrbg.webp')] bg-cover mask-[linear-gradient(to_bottom,transparent_0%,black_5%)]"
          style={{ zIndex: 0 }}
        />

        {/* 3. Foreground Content (stays sharp) */}
        <div className="relative z-10 flex flex-col items-center w-full">
          <div className="flex items-center gap-2 mt-10">
            <div className="flex max-xs:max-w-screen w-fit rounded-md border-(--white)/5 backdrop-blur-xs border-4">
              <p className="bg-[#00111fd3] max-xs:wrap-break-word px-3 py-2 rounded-md noto-sans text-sm max-xs:text-xs text-(--gray)">
                https://<span className="text-(--white)">monoshare.site</span>/
          ...
          <div className="relative p-1 mt-5 mx-8 rounded-md overflow-hidden group">
            ...
            <p className="noto-sans relative backdrop-blur-xs bg-[#001525ce] text-sm text-center text-(--gray) p-1 py-1.5 rounded-sm">
              You hold the key. Stored only in the generated url's hash, not
              even we can decrypt your secret.
            </p>
          </div>
        </div>
      </div>
...
const FreeUsageCard = () => {
  return (
    <m.div
      ...
      initial="initial"
      animate="show"
      className="flex flex-col w-120 max-xs:max-w-screen h-fit py-5 rounded-xl bg-[#cdd7df21] border-2 border-(--white)/5"
    >
      <p className="noto-sans text-(--gray)/80 text-sm px-5">
