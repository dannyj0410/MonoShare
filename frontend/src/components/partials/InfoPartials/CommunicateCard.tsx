import { useState } from "react";
import TypingInput from "./TypingInput";

const CommunicateCard = () => {
  const [animationKey, setAnimationKey] = useState(0);
  const handleReset = () => {
    setAnimationKey((prev) => prev + 1);
  };
  return (
    <div className="flex items-center">
      <div className="flex flex-col w-120 h-100 border border-[#042741] rounded-xl bg-linear-to-br from-[#0c325062] to-[#0804411c]">
        <p className="electrolize font-bold text-(--white) text-2xl pt-10 px-8 pb-2">
          Communicate Safely
        </p>
        <p className="noto-sans text-(--gray)/80 text-lg px-8">
          Stop sending sensitive data in plaintext. We ensure your information
          is permanently erased once it reaches the intended recipient.
        </p>

        <div className="flex gap-2 mx-8 mt-8 items-center justify-between">
          <TypingInput key={animationKey} />
          <button
            className="flex items-center justify-center bg-[#042741] hover:bg-[#052a46] h-full w-17 rounded-lg cursor-pointer duration-200 active:scale-95"
            onClick={handleReset}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 14 14"
              height="15"
              width="15"
            >
              <g id="send-email--mail-send-email-paper-airplane">
                <path
                  fill="#eee"
                  fillRule="evenodd"
                  d="M11.8214 0.0977331c0.2883 -0.1052509 0.6005 -0.1264465 0.9005 -0.0610159 0.3029 0.0660858 0.5805 0.2177638 0.7997 0.4370018 0.2192 0.219238 0.3709 0.496818 0.437 0.799741 0.0654 0.29992 0.0442 0.6122 -0.061 0.90043l-3.574 10.71201 -0.0002 0.0007c-0.0871 0.2634 -0.2404 0.5001 -0.4453 0.6873 -0.2043 0.1867 -0.45301 0.3178 -0.72238 0.381 -0.26942 0.0657 -0.55131 0.0601 -0.81798 -0.0161 -0.26643 -0.0761 -0.50848 -0.2201 -0.70245 -0.4179l-1.91734 -1.9086 -2.01406 1.0415c-0.15705 0.0812 -0.34532 0.0734 -0.49515 -0.0205 -0.14982 -0.0938 -0.23893 -0.2599 -0.23434 -0.4366l0.08257 -3.18376 7.04493 -5.11741c0.2793 -0.20286 0.3413 -0.59371 0.1384 -0.87298 -0.2029 -0.27928 -0.59371 -0.34122 -0.87299 -0.13836L2.20283 8.0884 0.473125 6.35869l-0.000058 -0.00005 -0.000126 -0.00013c-0.187323 -0.18719 -0.325225 -0.41797 -0.4013217 -0.67163 -0.07555546 -0.25185 -0.0878308 -0.51841 -0.0357814 -0.77609 0.0521166 -0.28151 0.1779581 -0.54421 0.3647391 -0.76128 0.18806 -0.21856 0.431073 -0.38293 0.703923 -0.47612l0.00337 -0.00115 0 0.00001L11.8214 0.0977331Z"
                  clipRule="evenodd"
                ></path>
              </g>
            </svg>
          </button>
        </div>

        {/*  keyboard UI */}
        <div className="mask-[linear-gradient(to_bottom,black_20%,transparent_100%)]">
          <div className="flex mx-12 mt-3 electrolize gap-3 items-center opacity-90">
            <div className="p-1.5 rounded-full bg-[#042741] hover:bg-[#042741a6] hover:rotate-90 duration-300">
              <svg
                className="w-3 h-3 -rotate-90"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="4"
                  d="M19 9l-7 7-7-7"
                ></path>
              </svg>
            </div>
            <p className="hover:bg-(--gray)/30 w-16 text-center rounded-sm">
              This
            </p>
            <div className="border-l border-(--gray)/50 h-5" />
            <p className="hover:bg-(--gray)/30 w-16 text-center rounded-sm">
              is
            </p>
            <div className="border-l border-(--gray)/50 h-5" />
            <p className="hover:bg-(--gray)/30 w-16 text-center rounded-sm">
              not
            </p>
            <div className="border-l border-(--gray)/50 h-5" />
            <p className="hover:bg-(--gray)/30 w-16 text-center rounded-sm">
              secure
            </p>
          </div>
          <div className="flex w-fit gap-2 justify-between ml-9 mr-11 pt-3 opacity-80">
            {"QWERTYUIOP".split("").map((key) => {
              return (
                <button
                  key={key}
                  className="bg-[#042741] border-[#093455] text-xs electrolize border-r-3 border-b-3 w-8 h-8 rounded-sm active:scale-95 duration-100"
                >
                  {key}
                </button>
              );
            })}
          </div>
          <div className="flex opacity-70 w-fit gap-2 justify-between mr-9 ml-11 pt-2">
            {"ASDFGHJKL;".split("").map((key) => {
              return (
                <button
                  key={key}
                  className="bg-[#042741] border-[#093455] text-xs electrolize border-r-3 border-b-3 w-8 h-8 rounded-sm"
                >
                  {key}
                </button>
              );
            })}
          </div>
        </div>
      </div>
    </div>
  );
};

// const CommunicateCard = () => {
//   return (
//     <div className="flex items-center">
//       <div className="flex flex-col w-120 h-100 border border-[#042741] rounded-xl bg-linear-to-br from-[#0c325062] to-[#0804411c] overflow-hidden">
//         <p className="electrolize font-bold text-(--white) text-2xl pt-10 px-8 pb-2">
//           Communicate Safely
//         </p>
//         <p className="noto-sans text-(--gray)/80 text-lg px-8">
//           Stop sending sensitive data in plaintext. We ensure your information
//           is permanently erased once it reaches the intended recipient.
//         </p>
//         <div className="flex gap-2 mx-8 mt-8 items-center justify-between">
//           <div className="flex w-full">
//             <input
//               type="text"
//               className="noto-sans tracking-tight text-(--gray) w-full px-4 py-2 rounded-lg border border-[#042741] bg-[#010407] outline-transparent focus:outline-[#14426631] focus:outline-2 outline-offset-1"
//               value="Your password is|"
//               readOnly
//             />
//           </div>
//           <button className="flex items-center justify-center bg-[#042741] hover:bg-[#052a46] h-full w-17 rounded-lg cursor-pointer duration-200 active:scale-95">
//             <svg
//               xmlns="http://www.w3.org/2000/svg"
//               fill="none"
//               viewBox="0 0 14 14"
//               id="Send-Email--Streamline-Core"
//               height="15"
//               width="15"
//             >
//               <desc>Send Email Streamline Icon: https://streamlinehq.com</desc>
//               <g id="send-email--mail-send-email-paper-airplane">
//                 <path
//                   id="Subtract"
//                   fill="#eee"
//                   fill-rule="evenodd"
//                   d="M11.8214 0.0977331c0.2883 -0.1052509 0.6005 -0.1264465 0.9005 -0.0610159 0.3029 0.0660858 0.5805 0.2177638 0.7997 0.4370018 0.2192 0.219238 0.3709 0.496818 0.437 0.799741 0.0654 0.29992 0.0442 0.6122 -0.061 0.90043l-3.574 10.71201 -0.0002 0.0007c-0.0871 0.2634 -0.2404 0.5001 -0.4453 0.6873 -0.2043 0.1867 -0.45301 0.3178 -0.72238 0.381 -0.26942 0.0657 -0.55131 0.0601 -0.81798 -0.0161 -0.26643 -0.0761 -0.50848 -0.2201 -0.70245 -0.4179l-1.91734 -1.9086 -2.01406 1.0415c-0.15705 0.0812 -0.34532 0.0734 -0.49515 -0.0205 -0.14982 -0.0938 -0.23893 -0.2599 -0.23434 -0.4366l0.08257 -3.18376 7.04493 -5.11741c0.2793 -0.20286 0.3413 -0.59371 0.1384 -0.87298 -0.2029 -0.27928 -0.59371 -0.34122 -0.87299 -0.13836L2.20283 8.0884 0.473125 6.35869l-0.000058 -0.00005 -0.000126 -0.00013c-0.187323 -0.18719 -0.325225 -0.41797 -0.4013217 -0.67163 -0.07555546 -0.25185 -0.0878308 -0.51841 -0.0357814 -0.77609 0.0521166 -0.28151 0.1779581 -0.54421 0.3647391 -0.76128 0.18806 -0.21856 0.431073 -0.38293 0.703923 -0.47612l0.00337 -0.00115 0 0.00001L11.8214 0.0977331Z"
//                   clip-rule="evenodd"
//                   stroke-width="1"
//                 ></path>
//               </g>
//             </svg>
//           </button>
//         </div>
//         <div className="mask-[linear-gradient(to_bottom,black_20%,transparent_100%)]">
//           <div className="flex mx-12 mt-3 electrolize gap-3 items-center opacity-90">
//             <div className="p-1.5 rounded-full bg-[#042741] hover:bg-[#042741a6] hover:rotate-90 duration-300">
//               <svg
//                 className="w-3 h-3 -rotate-90"
//                 fill="none"
//                 stroke="currentColor"
//                 viewBox="0 0 24 24"
//                 xmlns="http://www.w3.org/2000/svg"
//               >
//                 <path
//                   strokeLinecap="round"
//                   strokeLinejoin="round"
//                   strokeWidth="4"
//                   d="M19 9l-7 7-7-7"
//                 ></path>
//               </svg>
//             </div>
//             <p className="hover:bg-(--gray)/30 w-16 text-center rounded-sm">
//               This
//             </p>
//             <div className="border-l border-(--gray)/50 h-5" />
//             <p className="hover:bg-(--gray)/30 w-16 text-center rounded-sm">
//               is
//             </p>
//             <div className="border-l border-(--gray)/50 h-5" />
//             <p className="hover:bg-(--gray)/30 w-16 text-center rounded-sm">
//               not
//             </p>
//             <div className="border-l border-(--gray)/50 h-5" />
//             <p className="hover:bg-(--gray)/30 w-16 text-center rounded-sm">
//               secure
//             </p>
//           </div>
//           <div className="flex w-fit gap-2 justify-between ml-9 mr-11 pt-3 opacity-80">
//             {"QWERTYUIOP".split("").map((key) => {
//               return (
//                 <button className="bg-[#042741] border-[#093455] text-xs electrolize border-r-3 border-b-3 w-8 h-8 rounded-sm active:scale-95 duration-100">
//                   {key}
//                 </button>
//               );
//             })}
//           </div>
//           <div className="flex opacity-70 w-fit gap-2 justify-between mr-9 ml-11 pt-2">
//             {"ASDFGHJKL;".split("").map((key) => {
//               return (
//                 <button className="bg-[#042741] border-[#093455] text-xs electrolize border-r-3 border-b-3 w-8 h-8 rounded-sm">
//                   {key}
//                 </button>
//               );
//             })}
//           </div>
//         </div>
//       </div>
//     </div>
//   );
// };

export default CommunicateCard;
